...
Benchmark1::Benchmark1(Nodes nodes, Vehicles amvs, string modelname, TimeSpace_Network ts_network):Model(nodes, amvs, modelname, ts_network)
{
...
void Benchmark1::add_variables(GRBModel &mol)
{
    ...
    for(int a = 0; a < arc_type; a++)
    {
        ...
        for(int L = 0; L < arc_size; L++)
        {
            ...
            for(int p = 0; p < amv_set.veh_num; p++)
            {
                ...
                for(int k = 0; k < VTYPE; k++)
                {
                    f[a][L][p][k].resize(mav_num_type[k]+1);
                    for(int m = 1; m <= mav_num_type[k]; m++)
                    {
                        f[a][L][p][k][m] = mol.addVar(0.0, 1.0, 0, GRB_BINARY, 
                                "f" + to_string(a) + ",(" + to_string(s0) + "," + to_string(t0) + "," + to_string(s1) + "," + to_string(t1)+ ")," + to_string(p) + "," + to_string(k) + "," + to_string(m));
                    }
                    // if(k == 0)
                    // {
                    //     f[a][L][p][k].resize(pas_mav_num+1);
                    //     for(int m = 1; m <= pas_mav_num; m++)
                    //     {
                    //         f[a][L][p][k][m] = mol.addVar(0.0, 1.0, 0, GRB_BINARY, 
                    ...
                    // if(k == 1)
                    // {
                    //     f[a][L][p][k].resize(fre_mav_num+1);
                    //     for(int m = 1; m <= fre_mav_num; m++)
                    //     {
                    //         f[a][L][p][k][m] = mol.addVar(0.0, 1.0, 0, GRB_BINARY, 
                    //                 "f" + to_string(a) + ",(" + to_string(s0) + "," + to_string(t0) + "," + to_string(s1) + "," + to_string(t1)+ ")," + to_string(p) + "," + to_string(h));
                    //     }
...
void Benchmark1::add_objectives(GRBModel &mol)
{
...
void Benchmark1::add_constraints(GRBModel &mol)
{
    ...
    for(int i = 1; i < node_set.nodenum; i++)
    {
        if(node_set.nodetype[i] != 2) //if nodetype == 2 -> no serving arcs
        {
            ...
            mol.addConstr(serve_i_by_all_platoons <= 1, "servemost_" + to_string(i));
        }
    }
    ...
    //Constraint 16-19: relationship between f, z, y, and F, g, z
    for(int a = 0; a < network.get_tsarcs().size(); a++)
    {
        for(int L = 0; L < network.get_tsarcs()[a].size(); L++)
        {
            for(int p = 0; p < amv_set.veh_num; p++)
            {
                // GRBLinExpr total_mavs_on_arc;
                for(int k = 0; k < VTYPE; k++)
                {
                    for(int m = 1; m <= mav_num_type[k]; m++)
                    {
                        //! relationship between f, y, z
                        mol.addConstr(f[a][L][p][k][m] <= z[a][L][p], "fz_" + 
                                        to_string(a) + "," + to_string(L) + "," + to_string(p) + "," + to_string(k) + "," + to_string(m));
                        mol.addConstr(f[a][L][p][k][m] <= y[p][k][m], "fy_" + 
                                        to_string(a) + "," + to_string(L) + "," + to_string(p) + "," + to_string(k) + "," + to_string(m));
                        mol.addConstr(f[a][L][p][k][m] >= y[p][k][m] + z[a][L][p] - 1, "fyz_" + 
                                        to_string(a) + "," + to_string(L) + "," + to_string(p) + "," + to_string(k) + "," + to_string(m));
                        // total_mavs_on_arc += f[a][L][p][k][m] * m;
                    }
                }
                //! relationship between F, g, z
                //! express in one way
                for(int h = 1; h <= amv_set.max_plen; h++)
                {
                ...
                //! or express in the following way (F, g, z)
                // GRBLinExpr pl_len_on_arc;
                // for(int h = 1; h <= amv_set.max_plen; h++)
                // {
                //     pl_len_on_arc += F[a][L][p][h] * h;
                // }
                // mol.addConstr(total_mavs_on_arc == pl_len_on_arc, "fF_" + 
                //                 to_string(a) + "," + to_string(L) + "," + to_string(p));
            }
        }
    }

    //Constraint 20: AMV load limitation
    for(int a = 0; a < network.get_tsarcs().size(); a++)
    {
        for(int L = 0; L < network.get_tsarcs()[a].size(); L++)
        {
            for(int p = 0; p < amv_set.veh_num; p++)
            {
                for(int k = 0; k < VTYPE; k++)
                {
                    ...
                    for(int m = 1; m <= mav_num_type[k]; m++)
                    {
                        total_veh_k += f[a][L][p][k][m] * m;
                    }
                    mol.addConstr(E[a][L][p][k] <= veh_cap * total_veh_k, 
                                    "load_" + to_string(a) + "," + to_string(L) + "," + to_string(p) + "," + to_string(k));
                }
    ...
    //Constraint 21: initial vehicle load
    for(int p = 0; p < amv_set.veh_num; p++)
    {
        for(int k = 0; k < VTYPE; k++)
        {
    ...
    //Constraint 22: service amount related with serving arcs and customer demands
    for(int i = 1; i < node_set.nodenum; i++)
    {
        if(node_set.nodetype[i] != 2)
        {
    ...
    //Constraint 23: Flow balance of AMV load
    for(int p = 0; p < amv_set.veh_num; p++)
    {
        for(int k = 0; k < VTYPE; k++)
        {
...
void Benchmark1::get_result(GRBModel &mol)
{
    ...
    if (optimstatus == GRB_OPTIMAL) 
    {
        ...
        outfile << "print out the value of z, f and F:" << endl;
        for(int a = 0; a < network.get_tsarcs().size(); a++)
        {
            for(int L = 0; L < network.get_tsarcs()[a].size(); L++)
            {
                for(int p = 0; p < amv_set.veh_num; p++)
                {
                    ...
                    for(int k = 0; k < VTYPE; k++)
                    {
                        for(int m = 1; m <= mav_num_type[k]; m++)
                        {
                            if(f[a][L][p][k][m].get(GRB_DoubleAttr_X) > 0.5)
                            {
                                outfile << f[a][L][p][k][m].get(GRB_StringAttr_VarName) << " = "  << f[a][L][p][k][m].get(GRB_DoubleAttr_X) << endl;
                            }
                        }
                    }
                    for(int h = 1; h <= amv_set.max_plen; h++)
