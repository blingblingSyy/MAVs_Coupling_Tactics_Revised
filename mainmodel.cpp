...
MainModel::MainModel(Nodes nodes, Vehicles amvs, string modelname, TimeSpace_Network ts_network): Model(nodes, amvs, modelname, ts_network)
{

}

void MainModel::add_variables(GRBModel &mol)
{
    //x_{i,v}
    ...
    for(int a = 0; a < arc_type; a++)
    {
        ...
        for(int L = 0; L < arc_size; L++)
        {
            ...
            y[a][L].resize(amv_set.max_plen+1); //e.g., max_plen = 5 -> m = 1,2,3,4,5
            ...
            for(int m = 1; m <= amv_set.max_plen; m++)
            {
                y[a][L][m] = mol.addVar(0.0, 1.0, 0, GRB_BINARY, 
                        "y" + to_string(a) + ",(" + to_string(s0) + "," + to_string(t0) + "," + to_string(s1) + "," + to_string(t1)+ ")," + to_string(m));
            }
        }
    }
...
void MainModel::add_objectives(GRBModel &mol)
{
    ...
    {  
        ...
        for(int m = 1; m <= amv_set.max_plen; m++)  
        {    
            obj += w1 * node_set.initial_distance[s0][s1] * pl_factor(m) * y[0][L][m] * m; //! 0: arctype -> move
        }
    }
...
void MainModel::add_constraints(GRBModel &mol)
{
    ...
    for(int v = 0; v < amv_set.veh_num; v++)
    {
        ...
        mol.addConstr(moveout <= 1, "om_" + to_string(v));  //! moveout == 1
    }
    ...
    //Constraint 10: At most one length of platoon will travel along an arc
    for(int a = 0; a < network.get_tsarcs().size(); a++)
    {
        for(int L = 0; L < network.get_tsarcs()[a].size(); L++)
        {
            ...
            mol.addConstr(plen <= 1, "pl_" + to_string(a) + "," + to_string(L));
        }
    }

    //Constraint 11: The length of a platoon on an arc should be equal to the total number of AMVs involved on the same arc
    for(int a = 0; a < network.get_tsarcs().size(); a++)
    {
        for(int L = 0; L < network.get_tsarcs()[a].size(); L++)
        {
            GRBLinExpr vnum = 0.0, pl_len = 0.0;
            for(int m = 1; m <= amv_set.max_plen; m++)  
            {  
                pl_len += y[a][L][m] * m;
            }
            ...
            mol.addConstr(pl_len == vnum, "vehpl_" + to_string(a) + "," + to_string(L));                
        }
    }

    //Constraint 12: AMV load limitation
    for(int a = 0; a < network.get_tsarcs().size(); a++)
    {
        for(int L = 0; L < network.get_tsarcs()[a].size(); L++)
        {
    ...
    //Constraint 13: initial vehicle load
    for(int v = 0; v < amv_set.veh_num; v++)
    {
    ...
    //Constraint 14: service amount related with serving arcs and customer demands
    for(int i = 1; i < node_set.nodenum; i++)
    {
        if(node_set.nodetype[i] != 2)
        {
    ...
    //Constraint 15: Flow balance of AMV load
    for(int v = 0; v < amv_set.veh_num; v++)
    {
        for(int i = 1; i < node_set.nodenum; i++)
        {
...
void MainModel::get_result(GRBModel &mol)
{
    ...
    if (optimstatus == GRB_OPTIMAL) 
    {
        ...
        {  
            ...
            for(int m = 1; m <= amv_set.max_plen; m++)  
            {    
                obj_dist += node_set.initial_distance[s0][s1] * pl_factor(m) * y[0][L][m].get(GRB_DoubleAttr_X) * m; 
            }
        }
        ...
        for(int a = 0; a < network.get_tsarcs().size(); a++)
        {
            for(int L = 0; L < network.get_tsarcs()[a].size(); L++)
            {
                ...
                for(int m = 1; m <= amv_set.max_plen; m++)
                {
                    if(y[a][L][m].get(GRB_DoubleAttr_X) > 0.5)  //>=1
                    {
                        outfile << "  " << y[a][L][m].get(GRB_StringAttr_VarName) << " = "  << y[a][L][m].get(GRB_DoubleAttr_X) << endl;              
                    }
                }
            }
        }
